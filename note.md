...
再びマス目をクリックすると値が書き込まれなくなる。
ただし、個々のSquareコンポーネントではなくBoardコンポーネント内に保存されているので
Boardのstateが変更になると、個々のSquareコンポーネントも自動的に再レンダーされる。
全てのマス目の状態をBoardコンポーネント内で保持するようにしたことで、この後どっちが勝者かを判定できるようになる。

Square今ポーねとは自らでstateを管理しないようになったのでboardコンポーネントから値を受け取って
クリックされた徳はBoardコンポーネントに渡すだけになった
これをSquareコンポーネントは制御されたコンポーネント(controlled component)になったという
BoardがSquareコンポーネントを全体的に制御している状態になった。

`handleClick`内では、`squares`クォ直接変更する代わりに`.slice()`を呼んで配列のコピー作成をしている
